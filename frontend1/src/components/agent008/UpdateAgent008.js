...
function UpdateAgent008(props) {
    ...
    const loadProducts = async () => {
      const result = await axios.get(`http://localhost:8000/agent08/${id}`)
   ...
   const UpdateProductInfo = async () => {
    ...
    await axios({
        method: 'PUT',
        url: `http://localhost:8000/agent08/${id}/`,
        data: formField
    }).then(response => {
