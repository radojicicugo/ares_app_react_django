...
const getSingleProduct = async () =>{
 const { data } = await axios.get(`http://localhost:8000/agent001/${id}/`)
 console.log(data)
 setProduct(data)
}
...
const deleteProduct = async () => {
 await axios.delete(`http://localhost:8000/agent001/${id}/`)
 history('/')
}
    ...
    return (
        ...
        <Link to={`/${product.id}/updateagent001`} className="btn btn-primary m-2">Update</Link>
