...
function ShowAgent007(props) {
  ...
  console.log(props)
    ...
    const getProduct = async () => {
        const response = await axios.get("http://127.0.0.1:8000/agent07/")
...
return(
    <div className="showProduct-nav">
      <NavBarMenu/>
      <h3 className="ugljesa-h3">Agent 007</h3>
      <Link className="btn btn-success" to={'/addagent007'}>Dodaj Novo</Link>
        {
...
<Container className="map">
  ...
  <Card.Body className="body-body">
    ...
    <Link to={`/${el.id}/updateagent007`} className="btn btn-primary" >Detail</Link>
  </Card.Body>
</Card>
</Container>
            ))
