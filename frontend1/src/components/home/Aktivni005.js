...
const GetUgljesa = async () => {
  const {data} = await axios.get('http://localhost:8000/agent05/')
  console.log(data)
  setUgljesa(data) 
}
                    ...
                    <div>
{
    show ? (
      <div className="table" > 
<Table  key={index} className="box-aktivni" style={{background: <Aktivni005 /> ? "rgb(12, 255, 4)" : ""}} striped bordered hover responsive size="sm" >
<thead>
    <tr>
      <th>id</th>
      <th>Prezime</th>
