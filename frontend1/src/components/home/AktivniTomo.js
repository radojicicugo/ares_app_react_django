...
const GetUgljesa = async () => {
  const {data} = await axios.get('http://localhost:8000/tomo/')
  console.log(data)
  setUgljesa(data) 
}
                    ...
                    <div>
{
    show ? (
      <div className="table" > 
<Table  key={index} className="box-aktivni" style={{background: <AktivniTomo /> ? "rgb(32, 155, 30)" : ""}} striped bordered hover responsive size="sm" >
<thead>
    <tr>
      <th>id</th>
      <th>Prezime</th>
