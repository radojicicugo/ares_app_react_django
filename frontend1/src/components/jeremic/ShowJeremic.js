...
function ShowJeremic(props) {
    ...
    const getProduct = async () => {
        const response = await axios.get("http://127.0.0.1:8000/jeremic/")
...
return(
    <div className="showProduct-nav">
      <NavBarMenu/>
      <h3 className="ugljesa-h3">Jeremic</h3>
      <Link className="btn btn-success" to={'/addjeremic'}>Dodaj Novo</Link>
        {
...
<Container className="map">
  ...
  <Card.Body className="body-body">
    ...
    <Link to={`/${el.id}/updatejeremic/`} className="btn btn-primary" >Detail</Link>
  </Card.Body>
</Card>
</Container>
            ))
