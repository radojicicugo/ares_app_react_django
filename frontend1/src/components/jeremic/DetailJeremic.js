...
function DetailJeremic(props) {
   ...
   // const history = useHistory();
    ...
    const getSingleProduct = async () =>{
     const { data } = await axios.get(`http://localhost:8000/jeremic/${id}/`)
     console.log(data)
     setProduct(data)
    }
    ...
    const deleteProduct = async () => {
     await axios.delete(`http://localhost:8000/jeremic/${id}`)
     history('/chat')
    }
