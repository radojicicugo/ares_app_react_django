...
function DetailAngelovski(props) {
   ...
   // const history = useHistory();
    ...
    const getSingleProduct = async () =>{
     const { data } = await axios.get(`http://localhost:8000/angelovski/${id}/`)
     console.log(data)
     setProduct(data)
    }
    ...
    const deleteProduct = async () => {
     await axios.delete(`http://localhost:8000/angelovski/${id}`)
     history('/chat')
    }
...
export default DetailAngelovski;