...
const getSingleProduct = async () =>{
 const { data } = await axios.get(`http://localhost:8000/agent04/${id}/`)
 console.log(data)
 setProduct(data)
}
...
const deleteProduct = async () => {
 await axios.delete(`http://localhost:8000/agent04/${id}/`)
 history('/')
}
    ...
    return (
        ...
        <Link to={`/${product.id}/updateagent004`} className="btn btn-primary m-2">Update</Link>
        <Link className="btn btn-danger m-2" onClick={() => deleteProduct(product.id)}>Delete</Link>
        </div>
    );
}

export default DetailAgent004;