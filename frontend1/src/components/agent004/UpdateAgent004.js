...
function UpdateAgent004(props) {
    ...
    const loadProducts = async () => {
      const result = await axios.get(`http://localhost:8000/agent04/${id}`)
   ...
   const UpdateProductInfo = async () => {
    ...
    await axios({
        method: 'PUT',
        url: `http://localhost:8000/agent02/${id}/`,
        data: formField
    }).then(response => {
