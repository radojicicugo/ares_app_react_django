...
function ShowVane(props) {
  ...
  console.log(props)
    ...
    const getProduct = async () => {
        const response = await axios.get("http://127.0.0.1:8000/vane/")
...
return(
    <div className="showProduct-nav">
      <NavBarMenu/>
      <h3 className="ugljesa-h3">Vane</h3>
      <Link className="btn btn-success" to={'/addvane'}>Dodaj Novo</Link>
        {
...
<Container className="map">
  ...
  <Card.Body className="body-body">
    ...
    <Link to={`/${el.id}/updatevane`} className="btn btn-primary" >Detail</Link>
  </Card.Body>
</Card>
</Container>
            ))
