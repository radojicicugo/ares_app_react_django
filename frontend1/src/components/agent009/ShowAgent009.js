...
function ShowAgent009(props) {
  ...
  console.log(props)
    ...
    const getProduct = async () => {
        const response = await axios.get("http://127.0.0.1:8000/agent09/")
...
return(
    <div className="showProduct-nav">
      <NavBarMenu/>
      <h3 className="ugljesa-h3">Agent 009</h3>
      <Link className="btn btn-success" to={'/addagent009'}>Dodaj Novo</Link>
        {
...
<Container className="map">
  ...
  <Card.Body className="body-body">
    ...
    <Link to={`/${el.id}/updateagent009`} className="btn btn-primary" >Detail</Link>
  </Card.Body>
</Card>
</Container>
            ))
