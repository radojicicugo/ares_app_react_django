...
function ShowDejan(props) {
    ...
    const getProduct = async () => {
        const response = await axios.get("http://127.0.0.1:8000/dejan/")
...
return(
    <div className="showProduct-nav">
      <NavBarMenu/>
      <h3 className="ugljesa-h3">Dejan</h3>
      <Link className="btn btn-success" to={'/adddejan'}>Dodaj Novo</Link>
        {
            product.map((el, index) => (
<Container className="map">
  <Card className="card" style={{ width: '18rem'}} >
  <Card.Body className="body-body" key={index}>
    ...
    <Link to={`/${el.id}/updatedejan`}  className="btn btn-primary" >Detail</Link> 
