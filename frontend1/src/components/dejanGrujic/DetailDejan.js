...
function DetailDejan(props) {
   ...
   // const history = useHistory();
    ...
    const getSingleProduct = async () =>{
     const { data } = await axios.get(`http://localhost:8000/dejan/${id}/`)
     console.log(data)
     setProduct(data)
    }
    ...
    const deleteProduct = async () => {
     await axios.delete(`http://localhost:8000/dejan/${id}`)
     history('/chat')
    }
